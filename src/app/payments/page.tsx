...
type BulkSettlementAction = "paid" | "unpaid" | "pending" | "archive";

const bulkSettlementActionLabels: Record<BulkSettlementAction, string> = {
  paid: "Pago",
  unpaid: "Impago",
  pending: "Pendientes",
  archive: "Archivar",
...
export default function PaymentsPage() {
  ...
  const [bulkAction, setBulkAction] = React.useState<BulkSettlementAction>("paid");
  ...
  const handleArchiveSelected = async (settlementsToArchive: Settlement[]) => {
  ...
  const handleSelectRow = (id: string, checked: boolean) => {
    if (checked) {
      setSelectedSettlements(prev => prev.includes(id) ? prev : [...prev, id]);
    } else {
  ...
  const handleApplyBulkAction = async () => {
    const settlementsToProcess = settlements.filter((settlement) => selectedSettlements.includes(settlement.id));

    if (settlementsToProcess.length === 0) {
      toast({
        title: "Nada seleccionado",
        description: "Seleccione al menos una liquidacion para aplicar la accion masiva.",
      });
      return;
    }
    ...
    try {
      if (bulkAction === "archive") {
        await handleArchiveSelected(settlementsToProcess);
      } else {
        const batch = writeBatch(db);
        ...
        settlementsToProcess.forEach((settlement) => {
          ...
          if (bulkAction === "unpaid") {
            batch.update(settlementRef, {
              ...
              cashPaymentDate: deleteField(),
            });
            return;
          }

          batch.update(settlementRef, {
            status: getPendingStatusForSettlement(settlement),
            datePaid: deleteField(),
          });
        });
        ...
        toast({
          ...
          description: `Se actualizaron ${settlementsToProcess.length} liquidaciones a "${bulkSettlementActionLabels[bulkAction]}".`,
        });
      }
      ...
      if (bulkAction !== "archive") {
        toast({
          title: "Error",
          description: "No se pudo aplicar la accion masiva.",
          variant: "destructive",
        });
      }
    } finally {
  ...
  const canManageBulkActions = user?.role === 'admin' || user?.role === 'super-admin';

  const visibleSelectedCount = React.useMemo(
    () => filteredSettlements.filter((settlement) => selectedSettlements.includes(settlement.id)).length,
    [filteredSettlements, selectedSettlements]
  );

  const resetBulkEditing = React.useCallback(() => {
    setIsBulkEditing(false);
    setSelectedSettlements([]);
    setBulkAction("paid");
  }, []);

  const getPendingStatusForSettlement = React.useCallback((settlement: Settlement): Settlement["status"] => {
    if (settlement.receiptUrl) {
      return "Comprobante Subido";
    }
    if (settlement.cashPaymentDate) {
      return "Pago en Efectivo Agendado";
    }
    return "Pendiente";
  }, []);

  React.useEffect(() => {
    setSelectedSettlements((current) => current.filter((id) => filteredSettlements.some((settlement) => settlement.id === id)));
  }, [filteredSettlements]);
    ...
    >
        <DialogContent className="max-w-5xl">
            <DialogHeader>
                ...
                <DialogDescriptionComponent>
                    Mostrando las reservas incluidas en la liquidación para <strong>{viewingSettlement?.entityName}</strong> del período {viewingSettlement ? `${format(viewingSettlement.dateFrom, "dd/MM/yy")} - ${format(viewingSettlement.dateTo, "dd/MM/yy")}` : ''}.
                </DialogDescriptionComponent>
            </DialogHeader>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">Niños</TableHead>
                            ...
                            <div className="space-y-3 rounded-lg border p-4">
                                ...
                                {isSettlementEditable(viewingSettlement) && (
                                    <div className="space-y-2">
                                        ...
                                        />
                                        <Label htmlFor="adjustment-reason">Observación</Label>
                                        <Textarea
    ...
    <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
      <div className="flex items-center justify-between space-y-2">
        ...
        <div className="flex items-center gap-2">
            {isBulkEditing ? (
                <>
                <Select value={bulkAction} onValueChange={(value) => setBulkAction(value as BulkSettlementAction)}>
                ...
                <Button onClick={handleApplyBulkAction} disabled={isSubmitting || selectedSettlements.length === 0}>
                    <Archive className="mr-2 h-4 w-4"/>
                    Aplicar ({selectedSettlements.length})
                </Button>
                <Button variant="ghost" onClick={resetBulkEditing}>Cancelar</Button>
                </>
            ) : (
      ...
      <Card>
        <CardHeader>
          ...
          {isBulkEditing && (
            <div className="flex flex-col gap-2 rounded-md border border-dashed p-3 text-sm md:flex-row md:items-center md:justify-between">
              <span className="text-muted-foreground">
                Seleccione todas, una o varias liquidaciones visibles y aplique una accion masiva.
              </span>
              <span className="font-medium">{selectedSettlements.length} seleccionada(s)</span>
            </div>
          )}
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                {isBulkEditing && canManageBulkActions && (
                    <TableHead className="w-[40px]">
                      <Checkbox
                        checked={
                          filteredSettlements.length > 0 && visibleSelectedCount === filteredSettlements.length
                            ? true
            ...
            <TableBody>
              {filteredSettlements.length === 0 ? (
                <TableRow>
                    <TableCell colSpan={isBulkEditing && canManageBulkActions ? 8 : 7} className="h-24 text-center">
