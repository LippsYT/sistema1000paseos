...
export default function ProvidersPage() {
  ...
  const [providerToReset, setProviderToReset] = React.useState<Provider | null>(null);
  ...
  const handleResetProviderDebt = async () => {
    if (!providerToReset) return;

    const currentTotalDebt = providerDebt.get(providerToReset.id) || 0;
    const currentManualDebt = providerToReset.debtToProvider || 0;
    const newManualDebt = currentManualDebt - currentTotalDebt;

    try {
        const providerRef = doc(db, "providers", providerToReset.id);
        await updateDoc(providerRef, { debtToProvider: newManualDebt });

        toast({
            title: "Saldo reiniciado",
            description: `La deuda total de ${providerToReset.name} se ajusto a $0.00.`,
        });
        await fetchAllData();
    } catch (error) {
        console.error("Error resetting provider debt", error);
        toast({ title: "Error", description: "No se pudo reiniciar la deuda total.", variant: "destructive" });
    } finally {
  ...
  return (
    ...
    <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
      ...
      <AlertDialog open={!!providerToReset} onOpenChange={() => setProviderToReset(null)}>
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle>Reeniciar saldo del proveedor?</AlertDialogTitle>
                <AlertDialogDescription>
                    Esta accion ajusta la deuda total a cero para <strong>{providerToReset?.name}</strong>. No elimina reservas ni pagos, solo compensa el saldo actual. Desea continuar?
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancelar</AlertDialogCancel>
                <AlertDialogAction onClick={handleResetProviderDebt}>
                    Si, reeniciar saldo
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
                ...
                <TableRow key={provider.id}>
                  ...
                  <TableCell className="text-right font-mono">
                    <div className="flex items-center justify-end gap-2">
                        ...
                        {user?.role === 'super-admin' && (
                        <Button variant="outline" size="sm" onClick={() => setProviderToReset(provider)}>
